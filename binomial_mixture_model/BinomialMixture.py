'''
The BinomialMixtureModel implements the expectation-maximization (EM)
algorithm to fit a mixture of binomial distributions to the training data.
'''
import os
import pandas as pd
import numpy as np
...
class BinomialMixture():
    ...
    The BinomialMixture.fit() method fits the given data, a number of
    (n,N) pairs, by a mixture of K Binomial Distributions. Here K needs to be
    specified by the parameter `n_components` by the user.

    The BinomialMixture.predict() method, taking in (n, N) pairs, predicts the
    posterior probabilities for each (n,N) pair belonging to any of the fitted
    Binomial-Distribution components.

    '''
    def __init__(self, n_components, tolerance=1e-5,
                 max_step=1000, verbose=True, random_state=None):
        '''
        ...
        tolerance: Float
                   the object fits the data by EM iteration. tolerance is used
                   to define one of the conditions for the iteration to stop.
                   This condition says that the iteration continues until the
                   change of the parameters within the current iteration is
        ...
        # Initialize the pi_list. pi_k is the prior probability of class k.
        ...
        # Initialize the theta_list. theta_k is the binomial probability in
        # the k-th binomial distribution.
    ...
    def calc_logL(self, N_ls, n_ls):
        ...
        # log_binom_mat is a matrix of shape (S,K).
        # Its element_{i,l} = log_Binomial(ni|Ni, theta_l).
        # log with natural base.
        log_binom_mat = (Binomial(
                             N_ls.reshape(S,1),
                             theta_list.reshape(1,self.K)
                             ).log_prob(n_ls.reshape(S,1))
                        )
        # mean_log_binom, the mean value of all elements in log_binom_mat.
        calibration = torch.mean(log_binom_mat)
        # binom_mat has shape (S,K), element_{i,l} = Binomial(ni|Ni, theta_l)
        binom_mat = torch.exp(log_binom_mat - calibration)
        # log_likelihood = sum_{i=1}^{S} log(prob_i), this is a real number
        log_likelihood = (
            S*calibration
            + torch.sum(torch.log(torch.matmul(binom_mat, pi_list)))
            )
    ...
    def calc_Posterior(self, N_ls, n_ls):
        ...
        theta_list = self.theta_list
        S = len(N_ls)
        # shape = (K,K). At m-th row, l-th column, the element is
        # theta_ratio_{m,l} = theta_l/theta_m
        theta_ratio = torch.div(theta_list.reshape(1,self.K),
                                theta_list.reshape(self.K,1))
        # shape = (K,K), element_{ml} = (1-theta_l)/(1-theta_m)
        unity_minus_theta_ratio = torch.div(
                                      (1e0 - theta_list).reshape(1,self.K),
                                      (1e0 - theta_list).reshape(self.K,1)
                                      )
        # shape = (K,K).
        # element_{m,l} = (theta_l/theta_m) * [(1-theta_l)/(1-theta_m)]
        mixed_ratio = torch.mul(theta_ratio, unity_minus_theta_ratio)

        # shape = (K,K,S) with
        # element_{m,l,i} = [(theta_l/theta_m)*(1-theta_l)/(1-theta_m)]^ni
        # its element won't be either 0 or infty
        # no matther whether theta_l > or < theta_m
        mixed_ratio_pow = torch.pow(theta_ratio.reshape(self.K, self.K, 1),
                                    n_ls)
        mixed_ratio_pow = torch.clamp(mixed_ratio_pow, min=0.0, max=1e15)
        # shape = (K,K,S) with
        # element_{m,l,i} = [ (1-theta_l)/(1-theta_m) ]^(Ni-2ni)
        # its element may be infty if theta_l<<theta_m,
        # or 0 if theta_l >> theta_m
        unity_minus_theta_ratio_pow = (
            torch.pow(unity_minus_theta_ratio.reshape(self.K,self.K,1),
                      N_ls-2.0*n_ls)
            )
        unity_minus_theta_ratio_pow = torch.clamp(unity_minus_theta_ratio_pow,
                                                  min=0.0, max=1e15)

        # In below, we multiply the element of mixed_ratio_pow and the element
        # of unity_minus_theta_ratio_pow, and there won't be nan caused
        # by 0*infty or infty*0 because the element in mixed_ratio_pow
        # won't be 0 or infty. In this way, we make sure there won't be
        # nan in Posterior.

        # element-wise multiply, pow_tensor has shape(K,K,S),
        # element_{m,l,i} = (theta_l/theta_m)^ni * [(1-theta_l)/(1-theta_m)]^(Ni-ni).
        # Note that torch.mul(a, b) would broadcast if a and b are different
        # in shape & they are broadcastable. If a and b are the same in shape,
        # torch.mul(a,b) would operate element-wise multiplication.
        pow_tensor = torch.mul(mixed_ratio_pow, unity_minus_theta_ratio_pow)
        # pi_ratio has shape (K,K) with element_{m,l} = pi_l/pi_m
        pi_ratio = torch.div(pi_list.reshape(1,self.K),
                             pi_list.reshape(self.K,1))
        # posterior probability tensor, Pzim = P(zi=m|ni,Theta_old)
        # shape (K,S), element_{m,i} = P(zi=m|ni,Theta_old)
        Posterior = torch.pow(
                        torch.matmul(pi_ratio.reshape(self.K,1,self.K),
                                     pow_tensor),
                        -1e0
                        ).reshape(self.K,S)
        return Posterior
    ...
    def calc_params(self, N_ls, n_ls, Posterior):
        ...
        -----
         ...
         Posterior: tensor of shape (K,S)
                    contains the posterior probabilites for the ith sample
                    belonging to the m-th Binomial component.
                    Here i=1,2..,K and m=1,2,..,K
        '''
        # update pi_list
        # torch.sum(tensor, n) sum over the n-th dimension of the tensor
        # e.g. if tensor'shape is (K,S) and n=1, the resulting tensor
        # has shape (K,). The m-th element is the sum_{i=1}^{S} tensor_{m,i}.
        pi_list = torch.sum(Posterior,1)/len(N_ls)
        # update theta_list
        theta_list = torch.div(torch.matmul(Posterior, n_ls),
                               torch.matmul(Posterior, N_ls))
    ...
    def initialize_pi_theta(self, N_ls, n_ls):
        ...
        # theta are the ratio at the random indice,
        # { ratio_ls[index_k] | k=1,2,...,K }
        ...
        acc_portion_ls = ((random_indice[1:]+random_indice[:-1])
                          / 2.0
                          / len(ratio_ls))
    ...
    def fit(self, N_ls, n_ls):
        ...
        # Initialize the change of log_likelihood named
        # `delta_log_likelihood` and the iteration step called `iter_step`
        ...
        # The iteration stops when either of the following two conditions
        # is broken first
        ...
        while cond_step & cond_params:
            ...
            self.pi_list, self.theta_list =  self.calc_params(N_ls,
                                                              n_ls,
                                                              Posterior)
            ...
            delta_log_likelihood = torch.norm(log_likelihood_new
                                              - log_likelihood)
            # update params
            params_old = params
    ...
    def p_value(self, N_ls_new, n_ls_new, side='right'):
        ...
        for i in range(len(N_ls_new)):
            ...
            if side == 'right':
                ...
                p_value = torch.matmul(pi_list,
                                       t.FloatTensor(sf_list + pmf_list))
                ...
                sf_list = binom.sf(n, N, theta_list)
                pmf_list = binom.pmf(n, N, theta_list)
                p_value_right = torch.matmul(pi_list,
                                             t.FloatTensor(sf_list + pmf_list))
                cmf_list = binom.cdf(n, N, theta_list)
